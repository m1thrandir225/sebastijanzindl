...
const {data: homePage, status: pageStatus} = useSanityQuery<HomePageProperties>(pageQuery)
...
<template>
  <div v-if="pageStatus == 'success' && homePage" class="grid grid-cols-1 lg:grid-cols-3 w-full h-full gap-8 py-4 lg:gap-4">
    <div
      ...
      <div class="my-4 font-sans text-lg text-neutral-900 dark:text-neutral-100">
        <SanityContent :blocks="homePage.shortIntroSection.shortIntro"/>
      </div>
      ...
      <div v-if="homePage.socialSection" class="my-4" >
        ...
        <div class="flex flex-col flex-wrap w-full gap-2 my-2 lg:flex-row">
          <NuxtLink
v-for="link in homePage.socialSection?.socialLinks" :key="link._key" :to="link.url" target="_blank"
...
v-if="homePage.favouriteToolsSection"
      ...
      <div
        ...
        <VTooltip v-for="tool in homePage.favouriteToolsSection.tools" :key="tool._key">
          <template #popper>
            {{ tool.title }}
          </template>
          <div
            class="relative z-[150] p-2 transition-colors duration-200 ease-in-out border-2 border-transparent rounded-lg hover:dark:bg-neutral-100/20 hover:dark:border-neutral-200/50 hover:border-neutral-800/50 hover:bg-neutral-600/20 flex flex-row items-center justify-center">
              <SanityImage  v-if="isDark"  :image="tool.lightImage" :asset-id="tool.lightImage.asset._ref" class="self-center w-10 h-10"  :alt="tool.lightImage.alt" />
              <SanityImage v-else class="self-center w-10 h-10" :image="tool.darkImage" :asset-id="tool.darkImage.asset._ref" />
          </div>
        </VTooltip>
      </div>
    </div>
